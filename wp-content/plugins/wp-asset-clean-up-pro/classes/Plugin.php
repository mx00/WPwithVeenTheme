...
class Plugin
{
	...
	{
		$this->errors['wpacu_lite_enabled']  = __('Starting from version 1.0.3, "'.WPACU_PLUGIN_TITLE.'" does not require anymore to have "Asset CleanUp Lite" plugin active as a parent plugin, as the PRO version now works independently. The LITE version was deactivated. Please do not keep both versions running at the same time.', 'wp-asset-clean-up');
	...
	// [wpacu_pro]
	/**
	 * Call this after all plugins are loaded as it doesn't matter
	 * if the extension is loaded before the parent plugin
	 */
	public function afterPluginsLoaded()
	{
		// Lite Plugin is enabled
		try {
			if ($this->isWpacuLiteActive()) {
				throw new \Exception($this->errors['wpacu_lite_enabled']);
			}
		} catch (\Exception $e) {
			$this->currentError = $this->errors['wpacu_lite_enabled'];
			add_action('admin_notices', array( $this, 'showError'));
			deactivate_plugins(WP_PLUGIN_DIR.'/wp-asset-clean-up/wpacu.php');
		}
	}
	// [/wpacu_pro]
...
HTACCESS;
	...
	/**
	 * Print it at the top of the Dashboard in error style to raise awareness to the user
	 */
	public function showError()
	{
		...
		<div class="error fade is-dismissible">
			<p>
				<strong><?php echo esc_html($this->currentError); ?></strong>
			</p>
		</div>
		<?php
	}
