...
SQL;
	...
	public function pageOverview()
	{
		...
		$getAllPluginsRules = PluginsManager::getPluginRulesFiltered(false);
		...
		if ( ! empty($getAllPluginsRules) ) {
            ...
            foreach ($allPlugins as $pluginPath => $pluginData) {
                ...
                if ( ! isset($getAllPluginsRules[$pluginPath]) ) {
                    continue;
                }

	            $pluginsWithRules[] = array('title' => $pluginData['Name'], 'path' => $pluginPath, 'rules' => $getAllPluginsRules[$pluginPath]);
            }

            usort($pluginsWithRules, static function($a, $b)
            {
