...
class HardcodedAssets
{
		...
		if (! empty($hardcodedMarkedForUnloadList)) {
			foreach ($hardcodedMarkedForUnloadList as $hardCodedType => $hardcodedHandles) {
				foreach ($hardcodedHandles as $hardcodedHandle) {
					...
					if (in_array($hardCodedType, array('wpacu_hardcoded_scripts_src', 'wpacu_hardcoded_scripts_inline'))
	...
	public static function wpacuGenerateHardcodedStyleData( $dataRowObj, $data )
	{
		...
		*/
		...
		if ( in_array( $dataHH['row']['obj']->handle, $dataHH['global_unload'][$assetType] ) ) {
			$dataHH['row']['global_unloaded'] = $dataHH['row']['is_group_unloaded'] = true;
		}

		// Mark it as unloaded - for the Current Post Type
		if ( $dataHH['bulk_unloaded_type'] && in_array( $dataHH['row']['obj']->handle,
				$dataHH['bulk_unloaded'][ $dataHH['bulk_unloaded_type'] ][$assetType] ) ) {
		...
		$isLoadExceptionPerPage = isset( $dataHH['load_exceptions'][$assetType] ) && in_array( $dataHH['row']['obj']->handle,
				$dataHH['load_exceptions'][$assetType] );
	...
	public static function getHardcodedUnloadList()
	{
		...
		$hardcodedUnloadList = Misc::filterList($hardcodedUnloadList);

		return $hardcodedUnloadList;
	}
	// [/wpacu_pro]
