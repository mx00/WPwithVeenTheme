...
class Positions
{
	...
	public static function doChanges($htmlSource)
	{
		...
		$htmlSource = str_replace('<assetcleanuplink', '<link', $htmlSource);
...
JS;
	...
	public function appendStylesMovedToNewPosition($htmlSource)
	{
			...
			foreach ($stylesPositionsTags as $newPosition => $htmlTags) {
				...
				if ($toReplace) {
					if ($newPosition === 'head') {
						...
						foreach ($htmlTags as $indexData => $htmlTag) {
							$cssGeneratedCode = self::extractFromGeneratedCssLinkTag($htmlTag);
		...
		$htmlSource = str_replace(array(self::DEL_STYLES_MOVED_FROM_BODY_TO_HEAD, self::DEL_STYLES_MOVED_FROM_HEAD_TO_BODY), '', $htmlSource);
